...
import { NavigationMenu, Role } from "../auth/role";
import {
  AccountUnit,
  AuthUser,
  ExternalContext,
...
export const getCurrentSession = () =>
  unwrap<CurrentSession>(apiClient.get("/auth/me"));
...
export function selectOrg(organizationId: string) {
  return unwrap<LoginResponse>(
    apiClient.post("/auth/select-org", { organizationId }),
  );
}
...
export function switchOrg(organizationId: string) {
  return unwrap<LoginResponse>(
    apiClient.post("/auth/switch-org", { organizationId }),
  );
}
